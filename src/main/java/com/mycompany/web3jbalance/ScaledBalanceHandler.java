...
public class ScaledBalanceHandler implements HttpHandler {

    public static final String PATH = "/ScaledBalance";

    @Override
    public void handle(HttpExchange conn) throws IOException {
        try {
            String[] pathParts = conn.getRequestURI().getPath().split("/");
            String publicKeyAdress = pathParts[2]; // partes[0] = "", partes[1] = "ScaledBalance"
    ...
    byte[] sendResponse(String publicKeyAdress) throws InterruptedException, ExecutionException, TimeoutException {
        ...
        return scaledBalance.toString().getBytes();
    }
}